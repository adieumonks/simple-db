...
func TestOneBlockSort(t *testing.T) {
	db, err := server.NewSimpleDBWithMetadata(path.Join(t.TempDir(), "oneblocksorttest"))
	if err != nil {
	...
	sp := materialize.NewOneBufferSortPlan(tx, tp, []string{"a"})
